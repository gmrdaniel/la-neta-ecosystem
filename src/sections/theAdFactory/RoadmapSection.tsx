...
const LANETA_ACCENTS = [
  { pillBg: 'rgba(102, 65, 237, 0.15)', pillColor: 'var(--laneta-purple)' },
  { pillBg: 'rgba(255, 71, 172, 0.15)', pillColor: 'var(--laneta-pink)' },
] as const
...
function PhaseRow({
  ...
  phase: { dayRange: string; title: string; description: string; detail: string }
  index: number
}) {
...
export function RoadmapSection() {
  ...
  const phases = t('roadmap.phases', { returnObjects: true }) as { dayRange: string; title: string; description: string; detail: string }[]
        ...
        <div className="pb-16 pt-8 md:pb-20 md:pt-10">
          ...
          <div className="mt-14 rounded-2xl border border-slate-200 bg-white px-6 py-8 text-center shadow-lg md:px-10 md:py-10">
            <p className="text-xl font-bold tracking-tight text-slate-800 md:text-2xl" style={{ color: 'var(--laneta-purple)' }}>
              {t('roadmap.closingTitle')}
            </p>
            <p className="mx-auto mt-2 max-w-md text-sm text-slate-600">
              {t('roadmap.closingBody')}
            </p>
          </div>
        </div>
      </FloatingCard>
